...
const LineChart2 = () => {
  ...
  const viewResult = (dato) => {
    ...
    if (dato == 24) return <span>{Rendimiento[24]}</span>;

    if (dato == null) return <span>{Rendimiento[24]}</span>;
  };
  ...
  return (
    <>
      ...
      <div className="p">
        <h3>Rendimiento: {viewResult(valueInput)} kg/ha</h3>
        <br />
        <h3 style={{ color: "rgb(255, 99, 132)" }}>
          Utilidad: {viewUtilidad(valueInput)} u$s/ha
        </h3>
        <br />
        <h3 style={{ color: "rgb(31, 12, 143)" }}>
          Rentabilidad: {viewRentabilidad(valueInput)} (Utilidad u$s/Inversión
          u$s)
        </h3>
      </div>
    </>
